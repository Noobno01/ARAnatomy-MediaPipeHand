...
public class ARTemplateMenuManager : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("Button that shows the description of a selected object.")]
    Button m_DescriptionButton;

    public Button descriptionButton
    {
        get => m_DescriptionButton;
        set => m_DescriptionButton = value;
    }
    [SerializeField]
    [Tooltip("Button that shows the description of a selected object.")]
    Button m_SetButton;
    ...
    void OnEnable()
    {
        ...
        m_DescriptionButton.onClick.AddListener(ShowObjectDescription);
        m_SetButton.onClick.AddListener(fetch);
    ...
    void Update()
    {
        ...
        if (m_ShowObjectMenu || m_ShowOptionsModal)
        {
            ...
            if (m_ShowObjectMenu)
            {
                ...
                m_DescriptionButton.gameObject.SetActive(false);
                m_SetButton.gameObject.SetActive(false);
            }
            else
            {
                m_DeleteButton.gameObject.SetActive(m_InteractionGroup?.focusInteractable != null);
                m_DescriptionButton.gameObject.SetActive(m_InteractionGroup?.focusInteractable != null); // Show description button when an object is selected
                m_SetButton.gameObject.SetActive(m_InteractionGroup?.focusInteractable != null);
            }
        ...
        else
        {
            ...
            m_DescriptionButton.gameObject.SetActive(m_InteractionGroup?.focusInteractable != null);
            m_DeleteButton.gameObject.SetActive(m_InteractionGroup?.focusInteractable != null);
            m_SetButton.gameObject.SetActive(m_InteractionGroup?.focusInteractable != null);
    ...
    public GameObject descriptionTextUI;
    private TMP_Text greetingText;// A reference to the GameObject containing the text UI

    public void ShowObjectDescription()
    {
        // Check if m_InteractionGroup is assigned
        if (m_InteractionGroup == null)
        {
            Debug.LogError("m_InteractionGroup is null! Make sure it's assigned in the inspector.");
            return;
        }

        var currentFocusedObject = m_InteractionGroup.focusInteractable;
        Debug.Log($"Currently focused on: {currentFocusedObject}");

        if (currentFocusedObject != null)
        {
            // Cast the interactable to XRBaseInteractable
            XRBaseInteractable baseInteractable = currentFocusedObject as XRBaseInteractable;
            Component focusedComponent = (Component)currentFocusedObject;
            GameObject targe = focusedComponent.gameObject;

            if (baseInteractable != null)
            {
                Debug.Log(targe.name);
                if (handLandmarkerRunner == null)
                {
                    Debug.LogError("handLandmarkerRunner is null! Make sure it's assigned in the inspector.");
                    return;
                }
                handLandmarkerRunner.SetObjectToScale(targe.name);
                // Access the ObjectDescription component of the currently focused object
                var description = baseInteractable.gameObject.GetComponent<ObjectDescription>();
                if (description == null)
                {
                    description = baseInteractable.gameObject.AddComponent<ObjectDescription>();  // Adds it if missing
                }

                if (description != null)
                {
                    // Ensure the description UI is active
                    if (descriptionTextUI != null)
                    {
                        descriptionTextUI.SetActive(true); // Activate the UI

                        // Find the Greeting Text in the descriptionTextUI (its child)
                        greetingText = descriptionTextUI.GetComponentInChildren<TMP_Text>();

                        if (greetingText != null)
                        {
                            // Set the text of the Greeting Text to the description's text
                            greetingText.text = description.DescriptionText;
                        }
                        else
                        {
                            Debug.LogWarning("Greeting Text not found in the descriptionTextUI.");
                        }
                    }
                    else
                    {
                        Debug.LogWarning("Description Text UI GameObject is not assigned.");
                    }
                }
                else
                {
                    Debug.LogWarning("Focused object does not have a description.");
                }
            }
            else
            {
                Debug.LogWarning("Failed to cast focusInteractable to XRBaseInteractable.");
            }
        }
        else
        {
            Debug.LogWarning("No object is currently focused.");
        }
    }
    public Button continueButton;
    public void HideDescriptionPrompt()
    {
        if (descriptionTextUI != null)
        {
            descriptionTextUI.SetActive(false); // Hide the entire description prompt
        }
    }
    ...
    public void fetch()
    {
        ...
        if (currentFocusedObject != null)
        {
            ...
            XRBaseInteractable baseInteractable = currentFocusedObject as XRBaseInteractable;
            Component focusedComponent = (Component)currentFocusedObject;
            GameObject target = focusedComponent.gameObject;
            ...
            if (baseInteractable != null)
            {
                ...
                handLandmarkerRunner.SetObjectToScale(target.name);
            }
            else
            {
                Debug.LogWarning("Failed to cast focusInteractable to XRBaseInteractable.");
